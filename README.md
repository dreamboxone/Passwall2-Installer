...
2. Runs `opkg update`
3. Safely replaces `dnsmasq` with `dnsmasq-full` (downloads first, removes second — router is never left without DNS)
...
- Enough free space in overlay (~15–30 MB depending on cores you install)
...
- **LuCI menu doesn't show Passwall2** → clear LuCI cache: `rm -rf /tmp/luci-*` then refresh the browser.
